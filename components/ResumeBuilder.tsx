...
const ResumeBuilder: React.FC<ResumeBuilderProps> = ({ user }) => {
    ...
    useEffect(() => {
        const savedResume = databaseService.getResume(user.id);
        if (savedResume) {
            setResume(savedResume);
        }
    }, [user.id]);

    // Handle responsive scaling of the A4 preview
    useEffect(() => {
        const handleResize = () => {
            if (previewContainerRef.current) {
                ...
                setPreviewScale(Math.min(newScale, 1.2)); // Cap scale for large monitors
            }
        };
    ...
    const ResumePreviewContent = () => (
        <div 
            className="bg-white text-slate-900 shadow-2xl mx-auto flex flex-col transition-all duration-500 origin-top"
            style={{ 
                width: '794px', 
                minHeight: '1123px', 
                padding: '64px',
                transform: `scale(${previewScale})`,
                marginBottom: `calc((1123px * ${previewScale}) - 1123px)` // Adjust for scaled height
            }}
        >
            {/* Document Header */}
            ...
            <section className="mb-10">
                ...
                <p className="text-[14px] leading-relaxed text-slate-700 text-justify font-medium">{resume.summary || 'Describe your professional journey and key achievements in the summary section to see them populated here.'}</p>
            </section>
            ...
            <section className="mb-10">
                ...
                {resume.experience.length === 0 && <p className="text-[12px] text-slate-400 italic">No professional experience added yet.</p>}
            ...
            <div className="mt-auto pt-10 flex justify-center opacity-10">
                <Icon name="logo" className="h-10 w-10 text-slate-900" />
            </div>
        </div>
    );
        ...
        <div className="py-8">
                ...
                <div className="flex items-center gap-2 md:gap-4 w-full md:w-auto">
                    ...
                    <div className="flex bg-slate-200 dark:bg-slate-800 p-1 rounded-xl lg:hidden flex-grow md:flex-grow-0">
                        <button 
                            ...
                            className={`flex-1 px-4 py-2 text-xs font-black uppercase tracking-widest rounded-lg transition-all ${activeTab === 'edit' ? 'bg-white dark:bg-slate-700 text-indigo-600 shadow-sm' : 'text-slate-500'}`}
                        >
                            Edit
                        </button>
                        <button 
                            onClick={() => setActiveTab('preview')}
                            className={`flex-1 px-4 py-2 text-xs font-black uppercase tracking-widest rounded-lg transition-all ${activeTab === 'preview' ? 'bg-white dark:bg-slate-700 text-indigo-600 shadow-sm' : 'text-slate-500'}`}
                        >
                            Preview
                        </button>
                    </div>
                    ...
                    <div className="hidden sm:flex items-center gap-4 bg-white dark:bg-slate-800 p-2 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                      ...
                      <Button onClick={handleSave} disabled={saveStatus === 'saved'} className="h-9">
                        Save
                      </Button>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                ...
                <div className={`space-y-8 h-fit ${activeTab === 'preview' ? 'hidden lg:block' : 'block'}`}>
                    ...
                    <section className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-4">
                        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-700 pb-4">
                            <div className="flex items-center gap-3">
                                <div className="bg-sky-50 dark:bg-sky-900/30 p-2 rounded-lg text-sky-600 dark:text-sky-400">
                                    <Icon name="analyzer" className="h-5 w-5" />
                                </div>
                                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Summary</h3>
                            </div>
                            <Button onClick={() => handleGenerateAI('summary')} isLoading={loadingSection === 'summary'} className="bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 h-8 text-xs font-bold">
                                AI Write
                            </Button>
                        </div>
                        <Textarea rows={4} value={resume.summary} onChange={e => handleChange('summary', e.target.value)} placeholder="Career goals and top strengths..." />
                    </section>

                    <section className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-6">
                        <div className="flex items-center gap-3 border-b border-slate-100 dark:border-slate-700 pb-4">
                            <div className="bg-amber-50 dark:bg-amber-900/30 p-2 rounded-lg text-amber-600 dark:text-amber-400">
                                <Icon name="education" className="h-5 w-5" />
                            </div>
                            <h3 className="text-lg font-bold text-slate-900 dark:text-white">Education</h3>
                        </div>
                        {resume.education.map((edu, index) => (
                            <div key={edu.id} className="p-5 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-700 space-y-4 relative group">
                                <button onClick={() => removeEntry('education', index)} className="absolute top-4 right-4 text-slate-400 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100">
                                    <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg>
                                </button>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <Input label="School" value={edu.school} onChange={e => handleNestedChange('education', index, 'school', e.target.value)} />
                                    <Input label="Degree" value={edu.degree} onChange={e => handleNestedChange('education', index, 'degree', e.target.value)} />
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <Input label="Start" value={edu.startDate} onChange={e => handleNestedChange('education', index, 'startDate', e.target.value)} />
                                    <Input label="End" value={edu.endDate} onChange={e => handleNestedChange('education', index, 'endDate', e.target.value)} />
                                </div>
                            </div>
                        ))}
                        <button onClick={() => addEntry('education')} className="w-full py-3 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl text-slate-400 hover:border-amber-500 hover:text-amber-500 dark:hover:text-amber-400 transition-all font-bold text-sm">
                            + Add Education
                        </button>
                    </section>
                    ...
                    <section className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-6">
                        ...
                        {resume.experience.map((exp, index) => (
                            <div key={exp.id} className="p-5 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-700 space-y-4 relative group">
                                <button onClick={() => removeEntry('experience', index)} className="absolute top-4 right-4 text-slate-400 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100">
                                    <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg>
                                </button>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <Button onClick={() => handleGenerateAI('responsibilities', '', index)} isLoading={loadingSection === `responsibilities-${index}`} className="bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 h-8 text-xs font-bold px-3 border border-indigo-100 dark:border-indigo-900/50">
                                    AI Bullets
                                </Button>
                            </div>
                        ))}
                        <button onClick={() => addEntry('experience')} className="w-full py-3 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl text-slate-400 hover:border-indigo-500 hover:text-indigo-500 dark:hover:text-indigo-400 transition-all font-bold text-sm">
                            + Add Experience
                        </button>
                    </section>
                    ...
                    <section className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-4">
                        ...
                        <Textarea rows={3} value={resume.skills} onChange={e => handleChange('skills', e.target.value)} placeholder="React, TypeScript, SEO..." />
                    </section>
                </div>
                ...
                <div 
                    ...
                    className={`sticky top-24 ${activeTab === 'edit' ? 'hidden lg:block' : 'block'}`}
                >
